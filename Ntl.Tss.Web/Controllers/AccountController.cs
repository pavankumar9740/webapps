...
using Ntl.Tss.Web.Models.Account;
using Ntl.Tss.Web.Models.External;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<string> Token(string clientId)
        {
            ...
            var token = await _tools.IssueClientJwtAsync(
                clientId: clientId,
                lifetime: 3600,
                //audiences: client.AllowedScopes.Select(x => x.Scope).ToArray(),
        ...
        public async Task<IActionResult> ConfirmEmail(string userName, string code)
        {
            ...
            var codeMatchingResult = await _userManager.ConfirmEmailAsync(user, code);
            if (codeMatchingResult.Succeeded)
            {
                ...
                var resetPasswordUrl = Url.Action("ResetPassword", "Account", new { code = resetPasswordCode });
        ...
        {
            if (code == null)
                return View("ResetPasswordCodeInvalid");
            ...
            return View(new ResetPasswordModel { Input = inputModel });
        }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordModel resetPasswordModel)
        {
            ...
            var user = await _userManager.FindByEmailAsync(resetPasswordModel.Input.Email);
            if (user == null)
            {
            ...
            var result = await _userManager.ResetPasswordAsync(user, resetPasswordModel.Input.Code, resetPasswordModel.Input.Password);
            if (!result.Succeeded)
            {
                return View("ResetPasswordFailed");
            }
        ...
        private NotificationModel BuildResetPasswordNotificationModel(ResetPasswordNotificationModel resetPasswordNotificationModel)
        {
            var resetPasswordNotificationMessage = new ManufacturerPartialRegistrationMessage
            {
                ...
                To = new List<string> { resetPasswordNotificationModel.ToEmail },
